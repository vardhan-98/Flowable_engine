        ...
        try {
            ...
            if (resp.getStatusCode().is2xxSuccessful()) {
                elasticsearchService.logEvent(flowId, deviceId, "DeviceUpgrade", "vnf-spinup", "SUCCESS",
                        "VNF spin-up API responded with status " + resp.getStatusCodeValue());
            } else {
                elasticsearchService.logEvent(flowId, deviceId, "DeviceUpgrade", "vnf-spinup", "FAILED",
                        "VNF spin-up API responded with non-2xx status " + resp.getStatusCodeValue());
                throw new RuntimeException("VnfSpinUpDelegate failed with status " + resp.getStatusCodeValue());
            }
        } catch (Exception ex) {
            elasticsearchService.logEvent(flowId, deviceId, "DeviceUpgrade", "vnf-spinup", "FAILED",
