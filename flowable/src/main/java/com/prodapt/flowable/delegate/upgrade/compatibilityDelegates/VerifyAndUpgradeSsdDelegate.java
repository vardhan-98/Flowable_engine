...
public class VerifyAndUpgradeSsdDelegate implements JavaDelegate {
    ...
    public void execute(DelegateExecution execution) {
        ...
        try {
            elasticsearchService.logEvent(flowId, deviceId, "DeviceUpgrade", "verify-and-upgrade-ssd", "STARTED",
                    "Verifying and upgrading SSD via " + url);

            var requestBody = java.util.Map.of(
                "flowInstanceID", flowId,
                "deviceID", deviceId,
                "step", "verify-and-upgrade-ssd"
            );
            ...
            if (resp.getStatusCode().is2xxSuccessful()) {
                elasticsearchService.logEvent(flowId, deviceId, "DeviceUpgrade", "verify-and-upgrade-ssd", "SUCCESS",
                        "SSD verification and upgrade API responded with status " + resp.getStatusCode().value());
            } else {
                elasticsearchService.logEvent(flowId, deviceId, "DeviceUpgrade", "verify-and-upgrade-ssd", "FAILED",
