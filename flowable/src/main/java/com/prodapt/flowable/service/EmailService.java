...
public class EmailService {
    ...
    public void sendSelfScheduleEmail(String to, String deviceId, String scheduleUntil, String flowId) {
        try {
            String from = (fromEmail != null && !fromEmail.isBlank())
                ? fromEmail
                : "no-reply@" + ((mailHost != null && !mailHost.isBlank()) ? mailHost : "localhost");
            ZonedDateTime scheduleDateTime = ZonedDateTime.parse(scheduleUntil);
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd MMM yyyy, hh:mm a z");
            String scheduleReadable = scheduleDateTime.format(formatter);
            MimeMessage message = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(message, true);
            helper.setFrom(from);
            helper.setTo(to);
            helper.setSubject("Schedule Your Device Upgrade: Flexware P2 to P3");
            String headerBase64 = getHeaderImageBase64();
            String htmlContent = "<html>" +
            "<head>" +
            "  <style>" +
            ...
            "    <div class='content'>" +
            "      <p>Dear Customer,</p>" +
            "      <p>Your device is eligible for an upgrade. Please schedule it at your convenience with the following details:</p>" +
            "      <ul>" +
            "        <li><strong>Hostname:</strong> " + deviceId + "</li>" +
            "        <li><strong>Model ID:</strong> nfx250_att_ls1_10_t</li>" +
            "        <li><strong>JunOS Version:</strong> 15.1X53-D470.5</li>" +
            ...
            "        <li>Future-ready with access to new Virtual Network Functions (VNFs)</li>" +
            "      </ul>" +
            "      <h3>Schedule Your Upgrade</h3>" +
            "      <p>To schedule the upgrade, use the button below.</p>" +
            "      <div class='button-container'>" +
            "        <a href='" + "http://192.168.5.52:8080/api/devices/schedule/" + flowId + "' class='button'>Schedule Your Upgrade</a>" +
            "      </div>" +
            "      <p>Please note that you must schedule by " + scheduleReadable + ". After this time, scheduling will no longer be available.</p>" +
            "      <p>Thank you for trusting AT&T to power your network.</p>" +
            "      <p>Best regards,</p>" +
            "      <p>Flexware Support Team</p>" +
            "      <p>AT&T FlexWare Support Team</p>" +
            ...
            helper.setText(htmlContent, true);
            mailSender.send(message);
            elasticsearchService.logEvent(flowId, deviceId, "DeviceUpgrade", "send-self-schedule-email", "SUCCESS", "Self-schedule email sent from " + from + " to " + to + " via host " + mailHost + " for device " + deviceId);
        } catch (Exception ex) {
            elasticsearchService.logEvent(flowId, deviceId, "DeviceUpgrade", "send-self-schedule-email", "FAILED", "Failed to send self-schedule email to " + to + ": " + ex.getMessage());
        }
    }

    public void sendReminderEmail(String to, String deviceId, String scheduledTime, String flowId) {
        try {
            String from = (fromEmail != null && !fromEmail.isBlank())
                ? fromEmail
                : "no-reply@" + ((mailHost != null && !mailHost.isBlank()) ? mailHost : "localhost");
            ...
            "        <li>Future-ready with access to new Virtual Network Functions (VNFs)</li>" +
            "      </ul>" +
            "      <p>If you need to reschedule, please contact support as soon as possible.</p>" +
            "      <p>Thank you for trusting AT&T to power your network.</p>" +
            "      <p>Best regards,</p>" +
            "      <p>Flexware Support Team</p>" +
            "      <p>AT&T FlexWare Support Team</p>" +
